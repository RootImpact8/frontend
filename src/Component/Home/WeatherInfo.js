...
class WeatherInfo extends Component {
    ...
    getWeatherImage = (condition, isDay) => {
        ...
        if (conditionLower.includes("구름 조금") || conditionLower.includes("구름 약간")) return isDay ? fewCloudsDay : fewCloudsNight;
        if (conditionLower.includes("흐림") || conditionLower.includes("구름 많음")) return cloudyDay;
        if (conditionLower.includes("강풍") || conditionLower.includes("바람")) return windyDay;
    ...
    getBackgroundGradient = (condition) => {
        ...
        if (conditionLower.includes("맑음")) return "linear-gradient(135deg, #629DEE, #A8DEE0)"; // 맑은 날
        if (conditionLower.includes("구름") || conditionLower.includes("흐림")) return "linear-gradient(135deg, #A4C8E1, #6E94B9)"; // 구름 많음
        if (conditionLower.includes("비")) return "linear-gradient(135deg, #5A75A0, #6E94B9)"; // 비 오는 날
        if (conditionLower.includes("뇌우")) return "linear-gradient(135deg, #293259, #5C77A2)"; // 뇌우
        if (conditionLower.includes("눈")) return "linear-gradient(135deg, #DDEFFF, #A9C7E3)"; // 눈 오는 날
    ...
    render() {
        ...
        return (
            <div className={style.weather_container} style={{ background: backgroundGradient }}>
                {/* 상단 날짜 정보 */}
                ...
                <div className={style.weather_main}>
                    ...
                    <div className={style.weather_details}>
                        ...
                        <p className={style.weather_sub}>
                            최고 {Math.round(weatherData.current.temp_c + 3)}° / 최저 {Math.round(weatherData.current.temp_c - 3)}° | 바람 {Math.round(weatherData.current.wind_kph)} km/h
                        </p>
                    </div>

                    {/* 날씨 아이콘 (크기 키움) */}
                    <div className={style.weather_icon}>
                        <img src={weatherImage} alt={conditionText} className={style.weather_image} />
                    </div>
                </div>
                ...
                <p className={style.weather_tip}>
                    TIP: 오늘 날씨는 농작물 심기에 {weatherData.current.temp_c > 15 ? "적합합니다." : "다소 쌀쌀합니다."}
                </p>
            </div>
        );
    }
